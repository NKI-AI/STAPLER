...
Inside the `scripts` directory contains the following files to pre-train, fine-tune and/or test STAPLER on a <a href="https://slurm.schedmd.com" target="_blank">SLURM</a> cluster. Also provide an argument to `--partition` to specify the partition to use.
...
If you prefer to use bazel, all these targets are also available as bazel targets:

- `bazelisk run //scripts:pretrain`: Pre-train STAPLER.
- `python run //scripts:train_5_fold`: Fine-tune STAPLER using 5-fold cross-validation.
- `python run //scripts:test`: Test on a test set using a fine-tuned model checkpoint.
